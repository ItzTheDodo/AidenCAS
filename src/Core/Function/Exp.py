...
class Exp(Function):
    """Builtin exponential function exp(x)."""

    def __init__(self, namespace: Namespace | None = None):
        super().__init__("exp: R -> R; x -> x", namespace)
    ...
    def derivative(self, variable: str) -> Function:
        ...
        return self.namespace.cache_derivative(
            ...
            variable,
            "_d",
            Function(f"{self.name}_d_{variable}: R -> R; x -> exp(x)", self.namespace),
        )
    ...
    def inverse(self, value_name: str = "y") -> Function:
        return Function(f"{self.name}_inverse: R+ -> R; {value_name} -> log({value_name})", self.namespace)
